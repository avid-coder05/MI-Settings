...
public class LocalBluetoothProfileManager {
    ...
    public class StateChangedHandler implements BluetoothEventManager.Handler {
        ...
        protected void onReceiveInternal(Intent intent, CachedBluetoothDevice cachedBluetoothDevice) {
            ...
            if ((this.mProfile instanceof HeadsetProfile) && cachedBluetoothDevice.getDevice().getType() == 2 && cachedBluetoothDevice.getDevice().isDualModeDevice()) {
                Log.d("LocalBluetoothProfileManager", "HeadsetStateChangeHandler,Fake HFP broadcast,return");
                return;
            }
            ...
            if (cachedBluetoothDevice.getHiSyncId() == 0 || !LocalBluetoothProfileManager.this.mDeviceManager.onProfileConnectionStateChangedIfProcessed(cachedBluetoothDevice, intExtra)) {
                ...
                LocalBluetoothManager localBluetoothManager = LocalBluetoothManager.getInstance(LocalBluetoothProfileManager.this.mContext, null);
                BluetoothAdapter defaultAdapter = BluetoothAdapter.getDefaultAdapter();
                LocalBluetoothProfileManager profileManager = localBluetoothManager.getProfileManager();
                if (defaultAdapter == null || profileManager == null || !LocalBluetoothProfileManager.isTbsProfileEnabled()) {
                    return;
                }
                String findBrAddress = cachedBluetoothDevice.findBrAddress();
                if (findBrAddress == null || "".equals(findBrAddress)) {
                    Log.d("LocalBluetoothProfileManager", "onReceiveInternal : Br address is null, return");
                    return;
                }
                BluetoothDevice remoteDevice = defaultAdapter.getRemoteDevice(findBrAddress);
                if (remoteDevice == null) {
                    Log.d("LocalBluetoothProfileManager", "onReceiveInternal : Br device is null, return");
                    return;
                }
                CachedBluetoothDevice findDevice = LocalBluetoothProfileManager.this.mDeviceManager.findDevice(remoteDevice);
                if (findDevice == null) {
                    Log.d("LocalBluetoothProfileManager", "onReceiveInternal : Br cached device is null, new one");
                    findDevice = new CachedBluetoothDevice(LocalBluetoothProfileManager.this.mContext, profileManager, remoteDevice);
                }
    ...
    public List<BluetoothDevice> getLeAudioConnectedDevices() {
        LeAudioProfile leAudioProfile = this.mLeAudioProfile;
        return leAudioProfile != null ? leAudioProfile.getConnectedDevices() : new ArrayList(0);
    }

    public int getLeAudioConnectionStatus(BluetoothDevice bluetoothDevice) {
        LeAudioProfile leAudioProfile = this.mLeAudioProfile;
        if (leAudioProfile == null || bluetoothDevice == null) {
            return 0;
        }
        return leAudioProfile.getConnectionStatus(bluetoothDevice);
    }
    ...
    public void updateLocalProfiles() {
        ...
        if (this.mLeAudioProfile == null && supportedProfiles.contains(Integer.valueOf(BluetoothLeAudioFactory.getInstance().getLeAudioProfileId()))) {
            ...
            addProfile(leAudioProfile, "LE_AUDIO", BluetoothLeAudioFactory.getInstance().getLeAudioConnectionStateChangedAction());
        }
        if (this.mVolumeControlProfile == null && supportedProfiles.contains(Integer.valueOf(BluetoothLeAudioFactory.getInstance().getVcProfileId()))) {
            ...
            addProfile(volumeControlProfile, "VolumeControl", BluetoothLeAudioFactory.getInstance().getVcConnectionStateChangedAction());
        }
    ...
    public synchronized void updateProfiles(ParcelUuid[] parcelUuidArr, ParcelUuid[] parcelUuidArr2, Collection<LocalBluetoothProfile> collection, Collection<LocalBluetoothProfile> collection2, boolean z, BluetoothDevice bluetoothDevice) {
        ...
        if (ArrayUtils.contains(parcelUuidArr, BluetoothLeAudioFactory.getInstance().getLeAudioUuid()) && (leAudioProfile = this.mLeAudioProfile) != null) {
            collection.add(leAudioProfile);
            collection2.remove(this.mLeAudioProfile);
            this.mLeAudioProfile.setPreferred(bluetoothDevice, true);
        }
        if (ArrayUtils.contains(parcelUuidArr, BluetoothLeAudioFactory.getInstance().getVcUuid()) && (volumeControlProfile = this.mVolumeControlProfile) != null) {
            collection.add(volumeControlProfile);
            collection2.remove(this.mVolumeControlProfile);
            this.mVolumeControlProfile.setPreferred(bluetoothDevice, true);
        }
