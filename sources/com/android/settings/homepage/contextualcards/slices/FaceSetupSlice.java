...
public class FaceSetupSlice implements CustomSliceable {
    ...
    private static ListBuilder.RowBuilder buildRowBuilder(CharSequence charSequence, CharSequence charSequence2, IconCompat iconCompat, Context context, Intent intent) {
        return new ListBuilder.RowBuilder().setTitleItem(iconCompat, 0).setTitle(charSequence).setSubtitle(charSequence2).setPrimaryAction(SliceAction.createDeeplink(PendingIntent.getActivity(context, 0, intent, MiuiWindowManager$LayoutParams.EXTRA_FLAG_FULLSCREEN_BLURSURFACE), iconCompat, 0, charSequence));
    }
    ...
    public Intent getIntent() {
        return !this.mFaceManager.hasEnrolledTemplates(UserHandle.myUserId()) ? SliceBuilderUtils.buildSearchResultPageIntent(this.mContext, SecuritySettings.class.getName(), FaceStatusPreferenceController.KEY_FACE_SETTINGS, this.mContext.getText(R.string.security_settings_face_settings_enroll).toString(), 1401).setClassName(this.mContext.getPackageName(), SubSettings.class.getName()) : new Intent(this.mContext, FaceReEnrollDialog.class);
    }
