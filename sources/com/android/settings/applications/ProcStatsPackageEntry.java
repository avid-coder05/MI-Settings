...
public class ProcStatsPackageEntry implements Parcelable {
    ...
    public static CharSequence getFrequency(float f, Context context) {
        return f > 0.95f ? context.getString(R.string.always_running, Utils.formatPercentage((int) (f * 100.0f))) : f > 0.25f ? context.getString(R.string.sometimes_running, Utils.formatPercentage((int) (f * 100.0f))) : context.getString(R.string.rarely_running, Utils.formatPercentage((int) (f * 100.0f)));
    }
    ...
    public void retrieveUiData(Context context, PackageManager packageManager) {
        ...
        try {
            if ("os".equals(str)) {
                this.mUiTargetApp = packageManager.getApplicationInfo(ThemeResources.FRAMEWORK_PACKAGE, 4227584);
