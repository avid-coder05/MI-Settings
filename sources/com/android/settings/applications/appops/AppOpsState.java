...
public class AppOpsState {
    ...
    public static class AppOpEntry {
        ...
        public CharSequence getTimeText(Resources resources, boolean z) {
            return isRunning() ? resources.getText(R.string.app_ops_running) : getTime() > 0 ? DateUtils.getRelativeTimeSpanString(getTime(), System.currentTimeMillis(), 60000L, 262144) : z ? resources.getText(R.string.app_ops_never_used) : "";
        }
    ...
    public List<AppOpEntry> buildState(OpsTemplate opsTemplate, int i, String str, Comparator<AppOpEntry> comparator) {
        ...
        List opsForPackage = str != null ? appOpsState.mAppOps.getOpsForPackage(i, str, iArr) : appOpsState.mAppOps.getPackagesForOps(iArr);
        AppOpsManager.PackageOps packageOps3 = null;
        if (opsForPackage != null) {
            int i9 = 0;
            while (i9 < opsForPackage.size()) {
                AppOpsManager.PackageOps packageOps4 = (AppOpsManager.PackageOps) opsForPackage.get(i9);
        ...
        List<PackageInfo> list4 = packagesHoldingPermissions;
        int i11 = 0;
        while (i11 < list4.size()) {
            PackageInfo packageInfo3 = list4.get(i11);
