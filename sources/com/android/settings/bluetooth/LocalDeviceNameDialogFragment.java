    ...
    protected void setDeviceName(String str) {
        try {
            if (TextUtils.isEmpty(str)) {
                ToastUtil.show(getContext().getApplicationContext(), R.string.error_name_empty, 0);
            } else if (str.getBytes().length > 31) {
                ToastUtil.show(getContext().getApplicationContext(), R.string.device_name_input_overlength, 0);
            } else if (TextUtils.equals(getDeviceName(), str)) {
            } else {
                if (MiuiBTUtils.isCustomizedOperator()) {
                    MiuiBTUtils.setBluetoothName(str);
                } else {
                    MiuiUtils.setDeviceName(getActivity(), str);
                }
