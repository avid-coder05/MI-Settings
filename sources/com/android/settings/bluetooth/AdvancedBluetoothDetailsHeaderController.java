...
public class AdvancedBluetoothDetailsHeaderController extends BasePreferenceController implements LifecycleObserver, OnStart, OnStop, OnDestroy, CachedBluetoothDevice.Callback {
    ...
    public AdvancedBluetoothDetailsHeaderController(Context context, String str) {
        ...
        this.mMetadataListener = new BluetoothAdapter.OnMetadataChangedListener() { // from class: com.android.settings.bluetooth.AdvancedBluetoothDetailsHeaderController.1
    ...
    public /* synthetic */ void lambda$showBatteryPredictionIfNecessary$0(int i, int i2, LinearLayout linearLayout) {
        Cursor query = this.mContext.getContentResolver().query(new Uri.Builder().scheme("content").authority(this.mContext.getString(R.string.config_battery_prediction_authority)).appendPath(PATH).appendPath(DATABASE_ID).appendPath(DATABASE_BLUETOOTH).appendQueryParameter("address", this.mCachedDevice.getAddress()).appendQueryParameter(QUERY_PARAMETER_BATTERY_ID, String.valueOf(i)).appendQueryParameter(QUERY_PARAMETER_BATTERY_LEVEL, String.valueOf(i2)).appendQueryParameter("timestamp", String.valueOf(System.currentTimeMillis())).build(), new String[]{BATTERY_ESTIMATE, ESTIMATE_READY}, null, null, null);
        if (query == null) {
    ...
    void refresh() {
        ...
        ((TextView) this.mLayoutPreference.findViewById(R.id.entity_header_summary)).setText(this.mCachedDevice.getConnectionSummary());
