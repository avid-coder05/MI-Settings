...
public class BatteryChartPreferenceController extends AbstractPreferenceController implements PreferenceControllerMixin, LifecycleObserver, OnCreate, OnDestroy, OnSaveInstanceState, BatteryChartView.OnSelectListener, OnResume, ExpandDividerPreference.OnExpandListener {
    ...
    final List<BatteryDiffEntry> mSystemEntries;
    int mTrapezoidIndex;
    ...
    public final class LoadAllItemsInfoTask extends AsyncTask<Void, Void, Map<Integer, List<BatteryDiffEntry>>> {
        ...
        public Map<Integer, List<BatteryDiffEntry>> doInBackground(Void... voidArr) {
            ...
            Iterator<List<BatteryDiffEntry>> it = indexedUsageMap.values().iterator();
            while (it.hasNext()) {
                it.next().forEach(new Consumer() { // from class: com.android.settings.fuelgauge.BatteryChartPreferenceController$LoadAllItemsInfoTask$$ExternalSyntheticLambda1
    ...
    private String getSlotInformation(boolean z, String str) {
        return str == null ? z ? this.mPrefContext.getString(R.string.battery_app_usage_for_past_24) : this.mPrefContext.getString(R.string.battery_system_usage_for_past_24) : z ? this.mPrefContext.getString(R.string.battery_app_usage_for, str) : this.mPrefContext.getString(R.string.battery_system_usage_for, str);
    }
    ...
    private void refreshExpandUi() {
        ...
        Iterator<BatteryDiffEntry> it = this.mSystemEntries.iterator();
        while (it.hasNext()) {
            Preference findPreference = this.mAppListPrefGroup.findPreference(it.next().mBatteryHistEntry.getKey());
    ...
    /* JADX INFO: Access modifiers changed from: private */
    /* renamed from: setBatteryChartViewInner  reason: merged with bridge method [inline-methods] */
