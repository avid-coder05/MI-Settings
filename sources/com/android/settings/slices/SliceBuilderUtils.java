...
public class SliceBuilderUtils {
    ...
    private static Set<String> buildSliceKeywords(SliceData sliceData) {
        ...
        if (keywords != null) {
            arraySet.addAll((List) Arrays.stream(keywords.split(",")).map(new Function() { // from class: com.android.settings.slices.SliceBuilderUtils$$ExternalSyntheticLambda0
    ...
    public static PendingIntent getContentPendingIntent(Context context, SliceData sliceData) {
        return PendingIntent.getActivity(context, 0, getContentIntent(context, sliceData), MiuiWindowManager$LayoutParams.EXTRA_FLAG_FULLSCREEN_BLURSURFACE);
    }
    ...
    public static Pair<Boolean, String> getPathData(Uri uri) {
        ...
        return new Pair<>(Boolean.valueOf(TextUtils.equals(PaymentManager.KEY_INTENT, split[1])), split[2]);
    }
