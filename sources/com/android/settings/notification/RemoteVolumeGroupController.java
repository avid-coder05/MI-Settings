        ...
        for (int i = 0; i < this.mPreferenceCategory.getPreferenceCount(); i += 2) {
            ...
            while (true) {
                if (!it.hasNext()) {
                    z = false;
                    break;
                } else if (TextUtils.equals(preference3.getKey(), it.next().getId())) {
                    z = true;
                    break;
                }
            }
        ...
        if (preference.getKey().startsWith(SWITCHER_PREFIX)) {
            for (RoutingSessionInfo routingSessionInfo : this.mRoutingSessionInfos) {
                if (TextUtils.equals(routingSessionInfo.getId(), preference.getKey().substring(15))) {
                    this.mContext.sendBroadcast(new Intent().setAction("com.android.systemui.action.LAUNCH_MEDIA_OUTPUT_DIALOG").setPackage(ThemeResources.SYSTEMUI_NAME).putExtra(ExtraNetwork.FIREWALL_PACKAGE_NAME, routingSessionInfo.getClientPackageName()));
                    return true;
                }
            }
            return false;
