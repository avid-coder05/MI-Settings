...
public class CreateShortcutPreferenceController extends BasePreferenceController {
    ...
    private static final Comparator<ResolveInfo> SHORTCUT_COMPARATOR = new Comparator() { // from class: com.android.settings.shortcut.CreateShortcutPreferenceController$$ExternalSyntheticLambda1
    ...
    private static Bitmap createIcon(Context context, ApplicationInfo applicationInfo, int i, int i2, int i3) {
        ...
        int makeMeasureSpec = View.MeasureSpec.makeMeasureSpec(i3, SearchUpdater.SIM);
    ...
    private static ShortcutInfo createShortcutInfo(Context context, Intent intent, ResolveInfo resolveInfo, CharSequence charSequence) {
        ...
        return new ShortcutInfo.Builder(context, SHORTCUT_ID_PREFIX + intent.getComponent().flattenToShortString()).setShortLabel(charSequence).setIntent(intent).setIcon((i == 0 || (applicationInfo = activityInfo.applicationInfo) == null) ? Icon.createWithResource(context, R.drawable.ic_launcher_settings) : Icon.createWithAdaptiveBitmap(createIcon(context, applicationInfo, i, R.layout.shortcut_badge_maskable, context.getResources().getDimensionPixelSize(R.dimen.shortcut_size_maskable)))).build();
    }
