...
public class DiskLruCacheUtils {
    public static DiskLruCacheUtils instance;
    private static DiskLruCache lruCache;

    private DiskLruCacheUtils(Context context) {
        try {
            lruCache = DiskLruCache.open(new File(FileUtils.getCacheDirPath(context)), getAppVersion(context), 1, 10485760L);
        } catch (IOException e) {
    ...
    private int getAppVersion(Context context) {
        try {
            ...
            Log.e("DiskLruCacheUtils", "Cannot find package: ", e);
            return 1;
        }
    }

    public static synchronized DiskLruCacheUtils getInstance(Context context) {
        DiskLruCacheUtils diskLruCacheUtils;
        synchronized (DiskLruCacheUtils.class) {
            if (instance == null) {
                instance = new DiskLruCacheUtils(context.getApplicationContext());
            }
            diskLruCacheUtils = instance;
        }
