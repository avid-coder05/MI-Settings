...
public class MediaSessionCompat {
    ...
    public static class MediaSessionImplApi21 implements MediaSessionImpl {
        ...
        class ExtraSession extends IMediaSession.Stub {
            ...
            public void registerCallbackListener(IMediaControllerCallback cb) {
                ...
                MediaSessionImplApi21.this.mExtraControllerCallbacks.register(cb, new MediaSessionManager$RemoteUserInfo("android.media.session.MediaController", Binder.getCallingPid(), Binder.getCallingUid()));
            }
        ...
        MediaSessionImplApi21(MediaSession sessionFwk, VersionedParcelable session2Token, Bundle sessionInfo) {
            ...
            this.mToken = new Token(sessionFwk.getSessionToken(), new ExtraSession(), session2Token);
        ...
        public void sendSessionEvent(String event, Bundle extras) {
            if (Build.VERSION.SDK_INT < 23) {
                for (int beginBroadcast = this.mExtraControllerCallbacks.beginBroadcast() - 1; beginBroadcast >= 0; beginBroadcast--) {
                    try {
                        this.mExtraControllerCallbacks.getBroadcastItem(beginBroadcast).onEvent(event, extras);
                ...
                try {
                    switch (msg.what) {
                        ...
                        case 28:
                            ...
                            if (list != null) {
                                int i = msg.arg1;
                                QueueItem queueItem = (i < 0 || i >= list.size()) ? null : MediaSessionImplBase.this.mQueue.get(msg.arg1);
                                if (queueItem != null) {
        ...
        void postToHandler(int what, int arg1, int arg2, Object obj, Bundle extras) {
            synchronized (this.mLock) {
                ...
                if (messageHandler != null) {
                    ...
                    bundle.putInt(MediaSessionCompat.DATA_CALLING_UID, callingUid);
                    bundle.putString(MediaSessionCompat.DATA_CALLING_PACKAGE, getPackageNameForUid(callingUid));
                    int callingPid = Binder.getCallingPid();
                    if (callingPid > 0) {
                        bundle.putInt(MediaSessionCompat.DATA_CALLING_PID, callingPid);
                    } else {
                        bundle.putInt(MediaSessionCompat.DATA_CALLING_PID, -1);
                    }
                    if (extras != null) {
    ...
    public static final class Token implements Parcelable {
        public static final Parcelable.Creator<Token> CREATOR = new Parcelable.Creator<Token>() { // from class: android.support.v4.media.session.MediaSessionCompat.Token.1
            ...
            public Token createFromParcel(Parcel in) {
                return new Token(Build.VERSION.SDK_INT >= 21 ? in.readParcelable(null) : in.readStrongBinder());
            }
    ...
    public MediaSessionCompat(Context context, String tag, ComponentName mbrComponent, PendingIntent mbrIntent, Bundle sessionInfo, VersionedParcelable session2Token) {
        ...
        ComponentName componentName = mbrComponent;
        if (componentName != null && mbrIntent == null) {
            Intent intent = new Intent("android.intent.action.MEDIA_BUTTON");
            intent.setComponent(componentName);
            mbrIntent = PendingIntent.getBroadcast(context, 0, intent, 0);
        }
        PendingIntent pendingIntent = mbrIntent;
        int i = Build.VERSION.SDK_INT;
        if (i >= 21) {
            MediaSession createFwkMediaSession = createFwkMediaSession(context, tag, sessionInfo);
            if (i >= 29) {
                this.mImpl = new MediaSessionImplApi29(createFwkMediaSession, session2Token, sessionInfo);
            } else if (i >= 28) {
                this.mImpl = new MediaSessionImplApi28(createFwkMediaSession, session2Token, sessionInfo);
            } else {
                this.mImpl = new MediaSessionImplApi21(createFwkMediaSession, session2Token, sessionInfo);
            }
            setCallback(new Callback() { // from class: android.support.v4.media.session.MediaSessionCompat.1
            }, new Handler(Looper.myLooper() != null ? Looper.myLooper() : Looper.getMainLooper()));
            this.mImpl.setMediaButtonReceiver(pendingIntent);
        } else if (i >= 19) {
            this.mImpl = new MediaSessionImplApi19(context, tag, componentName, pendingIntent, session2Token, sessionInfo);
        } else if (i >= 18) {
            this.mImpl = new MediaSessionImplApi18(context, tag, componentName, pendingIntent, session2Token, sessionInfo);
        } else {
            this.mImpl = new MediaSessionImplBase(context, tag, componentName, pendingIntent, session2Token, sessionInfo);
        }
        this.mController = new MediaControllerCompat(context, this);
        if (sMaxBitmapSize == 0) {
    ...
    private MediaSession createFwkMediaSession(Context context, String tag, Bundle sessionInfo) {
        return Build.VERSION.SDK_INT >= 29 ? new MediaSession(context, tag, sessionInfo) : new MediaSession(context, tag);
    }
    ...
    public static MediaSessionCompat fromMediaSession(Context context, Object mediaSession) {
        int i = Build.VERSION.SDK_INT;
        if (i < 21 || context == null || mediaSession == null) {
            return null;
        }
        return new MediaSessionCompat(context, i >= 29 ? new MediaSessionImplApi29(mediaSession) : i >= 28 ? new MediaSessionImplApi28(mediaSession) : new MediaSessionImplApi21(mediaSession));
    }
        ...
        if (state != null) {
            ...
            if (state.getState() == 3 || state.getState() == 4 || state.getState() == 5) {
                if (state.getLastPositionUpdateTime() > 0) {
                    long elapsedRealtime = SystemClock.elapsedRealtime();
                    long playbackSpeed = (state.getPlaybackSpeed() * ((float) (elapsedRealtime - r0))) + state.getPosition();
